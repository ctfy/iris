...
public class TheContacts extends BaseTheAbout {
    ...
    private void syncContacts() {
        AsyncTask<Object, String, Object> task = new AsyncTask<Object, String, Object>() {
            public final static String HAS_UPDATE_CONTACT_INFO = "HAS_UPDATE_CONTACT_INFO ";
            boolean hasUpdateContactInfo = false;

            @Override
            protected void onPreExecute() {
                super.onPreExecute();
                hasUpdateContactInfo = TheApplication.getInstance().getSharedPreferences(TheContacts.class.getName(), Context.MODE_PRIVATE).contains(HAS_UPDATE_CONTACT_INFO);
            }

            @Override
            protected Object doInBackground(Object... params) {
                AppLog.d(TAG, hasUpdateContactInfo ? "联系人列表已经同步,跳过" : "联系人列表未同步过");
                if (!hasUpdateContactInfo) {
            ...
            protected void onPostExecute(Object result) {
                ...
                TheApplication.getInstance().getSharedPreferences(TheApps.class.getName(), Context.MODE_PRIVATE).edit().putBoolean(HAS_UPDATE_CONTACT_INFO, true).commit();
            }
        };
        task.execute(0);
    ...
    public static List<TbAppCache> query(String name) {
        return TbContactCache.queryContacts(name);
    }

    public static void deletePackage(String number) {
        TbContactCache.deleteContact(number);
    }

    public static List<String> getAllContactsName() {
        return TbContactCache.queryContacts();
    }
}
